...
export default function AdminStudentsPage() {
  ...
  async function handleSave() {
    ...
    if (editing) {
      const { error: err } = await supabase.from('profiles').update(payload).eq('id', editing.id);
      if (err) setError(err.message);
    } else {
      // For new students, we need to create via auth first
      setError('Vui lòng tạo tài khoản qua trang Đăng ký. Tính năng này yêu cầu tài khoản auth.');
      setSaving(false);
      return;
    }
    setSaving(false);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-border/50 bg-card p-6 shadow-xl">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-lg font-semibold">Sửa thông tin học viên</h3>
